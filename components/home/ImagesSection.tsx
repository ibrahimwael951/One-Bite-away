...
const ImagesSection = () => {
  ...
  return (
    <section>
      <div className=" hidden md:flex flex-col md:flex-row items-center justify-evenly  w-full min-h-[500px]  max-w-7xl mx-auto overflow-hidden">
        ...
        {imageGroups.map((group, i) => (
          <div key={i} className="relative h-[400px] w-full md:w-3/5 lg:w-2/5">
            {group.map((img, j) => (
              <DraggableImage key={j} alt="image" {...img}  />
            ))}
          </div>
        ))}
      </div>
      {/* mobile section */}
      <motion.div {...FadeUp} className="mt-5 w-full h-[500px] md:hidden ">
        <Swiper
          loop={true}
          slidesPerView="auto"
          spaceBetween={0}
